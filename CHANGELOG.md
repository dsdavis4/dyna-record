...
- **ID generation:** `uuid` package dropped from dependencies; ID defaults now generated by Node's built-in `globalThis.crypto.randomUUID()` (same v4 format, same string shape). Mocking-friendly via a thin `src/id.ts` indirection.

### Breaking

- **Node engine floor raised to `>=22`.** Drops support for Node <22 (Node 18 EOL'd in 2025; Node 20 EOL'd April 2026). Aligns with the AWS Lambda `nodejs22.x` runtime baseline.
