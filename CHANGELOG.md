...
- **Two search surfaces, fully inferred.** `Parent.search(scopeId, query, options)` (available when exactly one vector index is `scopedBy` the class) and `myIndex.search(...)` on the construct returned by the new `Table.vectorIndex({ name, model, provider, scopedBy?, include? })` static — always unambiguous, with the scope value first on scoped indexes. `in:` narrows to the parent's searchable relationship names (member entity names on the index construct) and result unions narrow accordingly; `filter` keys narrow to the searched entities' `@SearchFilterable()` attributes (strings, numbers, booleans, enums, and foreign keys). Search filters are an equality-only `AND` conjunction — an AWS grammar constraint, enforced at compile time and at runtime. A `scopedBy` index's `HASH` is the enforced isolation boundary (a tenant-scoped index cannot search across tenants); filterable foreign keys narrow within it in the same single operation. Results compile to exactly one `SearchVectors` operation and return complete typed entity instances with `similarity` (higher is better, converted per the model's distance function) and the raw AWS `score`.
