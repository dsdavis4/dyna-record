...
- **The unchanged-value embed skip is pinned to the searchable value it was decided on.** When an update skips the embedding call because the stored value matches, the canonical row's transaction carries the condition `attribute_exists(...) AND #<searchableAlias> = :<searchableAlias>`, reusing the SET's own expression name and value — the pin adds no request bytes. Without the pin, a concurrent write clearing the searchable value in the prefetch-to-commit window produced a row with searchable text but no vector — silently missing from the index, a state no serial ordering of the two updates can produce. That interleaving now fails the update loudly with a retryable `ConditionalCheckFailedError` (`... does not exist or its searchable value was changed by a concurrent write — retry the update`); a retry re-reads, sees the changed value, and embeds.
...
- **`HasAndBelongsToMany` link records no longer copy the embedding vector.** The join-table link `Put` spreads a prefetched canonical row; it now strips the vector the same way belongs-to link records always did. The vector lives on canonical rows only — no denormalized copy carries any vector-search data.
