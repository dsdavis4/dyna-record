...
- **`sideEffects: false`** declared in `package.json` — consumer bundlers can tree-shake unused exports more aggressively.

### Changed

- **TypeScript 6 (`^6.0.3`)** is the new minimum compiler requirement. The library's own source compiles under `module: Preserve`, `moduleResolution: bundler`, `target: ES2025`, with `types: ["node"]` set explicitly (TS 6 changed the default to `[]`). The `@tsconfig/recommended` extends has been dropped in favor of a fully-explicit `tsconfig.json` so settings are visible in one place.
- **Build tool: tsup (`^8.5.1`)** replaces direct `tsc` build. The published artifact is now a single bundled ESM file (`dist/index.js`) plus a single CJS file (`dist/index.cjs`), each with matching `.d.ts` / `.d.cts` declarations and sourcemaps. AWS SDK and zod remain external runtime dependencies (not inlined into the bundle).
- **Test runner: Vitest (`^3.2.4`)** replaces Jest + ts-jest. The test API surface used by consumers and contributors is near-identical (`describe`, `it`, `expect`, `vi.*` matching `jest.*`). Test reliability: pass count and per-suite breakdown match the pre-migration Jest baseline exactly (30 suites, 983 tests). Coverage instrumentation switched from Istanbul to V8 — actual numbers remain near-identical to the prior baseline; the same 85/60/85/85 thresholds (branches/functions/lines/statements) are enforced.
- **ID generation:** ID defaults now generated by Node's built-in `globalThis.crypto.randomUUID()` (same v4 format, same string shape). Mocking-friendly via a thin `src/id.ts` indirection.

### Removed

- **`uuid` dependency.** Not previously re-exported from the package root, so there is no consumer-visible type change. Replaced internally by Node's built-in `crypto.randomUUID()`.
...
- **CJS runtime consumption is supported for exports-aware resolvers** (Node 22+, current bundlers, Node 22+'s stabilized `require(esm)`). Non-exports-aware older resolvers (pre-2024 webpack 4, parcel 1, older Rollup) are best-effort and no longer a guaranteed contract.

### Migration

- Bump your CI / runtime to Node `>=22` (for AWS Lambda: switch to the `nodejs22.x` runtime).
- Bump your TypeScript dev-dependency to `>=6`.
- Replace any `import X from "dyna-record/dist/..."` with `import { X } from "dyna-record"` — every publicly-supported symbol is reachable from the root entry.
