...
- **DynamoDB native vector search.** Entities declare one searchable attribute with the layered `@Searchable()` decorator (and the `Searchable` property brand); creates and updates embed the value synchronously through a consumer-supplied embedding provider and write the vector plus a content hash onto the entity's canonical row inside the operation's existing single transaction — no second write, no eventual-consistency pipeline, and rows are searchable sub-second after the write acknowledges. Similarity searches compile to exactly one `SearchVectors` operation and return complete typed entity instances with `similarity` (higher is better, converted per the model's distance function) and the raw AWS `score`. Two search surfaces ship: `Parent.search(scopeId, query, options)` (available when exactly one vector index is `scopedBy` the class), and `myIndex.search(...)` on the construct returned by the new `Table.vectorIndex({ name, model, provider, scopedBy?, include? })` static — always unambiguous, with the scope value first on scoped indexes. Search options are fully inferred: `in:` narrows to the parent's searchable relationship names (member entity names on the index construct), result unions narrow accordingly, and `filter` keys narrow to the searched entities' `@SearchFilterable()` attributes (strings, numbers, booleans, enums, and foreign keys). Search filters are an equality-only `AND` conjunction — an AWS grammar constraint, enforced at compile time and at runtime. A `scopedBy` index's `HASH` is the enforced isolation boundary (a tenant-scoped index cannot search across tenants); filterable foreign keys narrow within it in the same single operation. The library ships **no embedding implementation and no embedding SDK dependency** — the required `provider` is an embed function you own (credentials, region, retry, timeout), paired with the shipped pure-data `TitanTextEmbedV2` model descriptor or your own `EmbeddingModelDescriptor`. Provider failures fail the whole write with the new `EmbeddingError` (provider error on `cause`); updates carrying an unchanged searchable value skip the embedding call via the stored content hash. See the README's new **Vector Search** section for the declaration walkthrough, provisioning contract, tenant-isolation guidance, and the honest cost model (vector write billing dominates at `max(1024, 4 × dimensions)` bytes per material write to a vector-bearing row).
- **New root exports.** Value exports: `Searchable`, `SearchFilterable` (decorators, previously unexported), `TitanTextEmbedV2`, `VectorIndexMetadata`, `vectorSearchKeys`, and the `EmbeddingError`/`FilterError` error classes. Type exports: `Searchable`/`SearchFilterable` (property brands), `EmbeddingProvider`, `EmbeddingModelDescriptor`, `VectorDistanceFunction`, `VectorIndexOptions`, `SearchQuery`, `SearchOptions`, `SearchResult`, `SearchResults`, `SearchFilter`, `SearchFilterParams`, `SearchFilterValue`, `SerializedTableMetadata`, `SerializedVectorIndexMetadata`, and every search inference helper (`SearchableRelationshipProperties`, `InferSearchResults`, `ParentSearchOptions`, `IndexSearchOptions`, and friends) — inference helpers are exported from the root at the time they are written and verified through real npm pack-and-resolve, per the 0.7.1 lesson about deep conditional types silently degrading to `any` behind the closed `exports` map.

### Changed

- **`metadata()` gains vector index fields — this is the IaC provisioning contract.** On tables that declare a vector index, the serialized table metadata now carries a `vectorIndexes` array (index name, model descriptor *name* only — never the provider, client config, or credentials — dimensions, distance function, `ALL` projection, the search schema as table aliases with the auto-declared entity `type` inline filter, a `scopedBy` entity name when scoped, and a stable configuration `fingerprint` so IaC can detect that a decorator change implies a destructive index replacement). The exported `SerializedTableMetadata` type gains these fields accordingly; consumers structurally typing against `metadata()`'s return shape should account for the new optional field. Tables without a vector index serialize exactly as before.
- **Ordinary reads on vector-indexed tables exclude the vector via projection.** `findById`, `query`, and the internal update/delete pre-fetches on a table that declares a vector index now send a `ProjectionExpression` covering every registered attribute alias except the vector — a bandwidth and latency saving (reads bill on full item size regardless of projection; the row is permanently larger, as the README's cost model spells out). Read commands on tables **without** a vector index are byte-identical to 1.0.1. Hydrated searchable entities additionally carry the library-managed `__dyna_vector_hash` string attribute (the content hash behind the unchanged-value embed skip); the vector itself never appears on hydrated entities.
...
- **The unchanged-value embed skip is pinned to the stored content hash.** When an update skips the embedding call because the stored hash matches, the canonical row's transaction now carries the condition `attribute_exists(...) AND #__dyna_vector_hash = :__dyna_vector_hash`. Without the pin, a concurrent write clearing the searchable value in the prefetch-to-commit window produced a row with searchable text but no vector — silently unindexed, a state no serial ordering of the two updates can produce. That interleaving now fails the update loudly with a retryable `ConditionalCheckFailedError` (`... does not exist or its searchable value was changed by a concurrent write — retry the update`); a retry re-reads, finds no stored hash, and embeds.
- **Metadata initialization validates two more vector-index invariants, failing fast with actionable messages.** Every member's scope foreign key must resolve to the same table alias — the scoped `HASH` equality is one table attribute (mirrors the existing `@SearchFilterable` alias-consistency check). And every vector index on a table must declare the same provider, model, dimensions, and distance function — all indexes share the vector attribute and writes embed once, so the write path's index selection is correct by construction.

### Fixed

- **Search runtime guards hardened against malformed and prototype-shaped input.** The `in:` option and filter keys are validated with own-property checks (`Object.hasOwn`), so inherited object keys (EX: `"constructor"`) are rejected with the intended `ValidationError`/`FilterError` instead of crashing with a `TypeError`. A `filter` that is empty or carries only explicitly-undefined values compiles to no condition instead of a malformed expression DynamoDB rejects at query time. A `null` filter condition value from an untyped caller is rejected by the value guard with a `FilterError` instead of a `TypeError`. And a global index called with the scoped signature shape (`search(scopeId, query, options)`) throws a `ValidationError` instead of silently embedding the scope id as the query text.
- **Denormalized copies carry no vector-search bookkeeping — neither the vector nor the content hash.** Every copy path is covered: `HasAndBelongsToMany` link records and belongs-to reverse links (raw spreads of prefetched canonical rows) strip both attributes explicitly, and entity serialization (`entityToTableItem`) never emits the content hash, so link records built from hydrated entities are clean too. Vector-search bookkeeping lives on canonical rows only, with no stale hashes visible on entities hydrated from copies. (Copies written by earlier pre-release builds of this branch may carry a leftover hash; it is inert and disappears the next time the copy is rewritten.)
- **A create's embedding failure can no longer surface as an unhandled promise rejection.** The embed starts concurrently with the belongs-to prefetch; a rejection absorber now covers that window, so a provider failure still fails the create through the awaited path (`EmbeddingError`) without also crashing the process when the prefetch fails first.
