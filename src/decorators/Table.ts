 ...
 * @returns A class decorator factory function that takes a target class extending `NoOrm` and a context object provided by the TypeScript runtime. The decorator function registers the provided metadata options with the ORM's metadata system, ensuring the entity is properly configured and recognized by the ORM.
 *
 * Usage example:
 * ```typescript
 * @Table({ name: 'my-table', delimiter: '"#",' })
 * class MyTable extends NoOrm {
